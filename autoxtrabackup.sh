...
usage () {
        ...
        echo -e "\t\t1: xtrabackup --decompress $backupDir/(daily/weekly/monthly)/BACKUP-DIR";
        echo -e "\t\t2: Follow same steps as for non-compressed backups";
        echo -e "\tRestore a non-compressed backup:";
        echo -e "\t\t1: xtrabackup --apply-log $backupDir/(daily/weekly/monthly)/BACKUP-DIR";
        echo -e "\t\t2: Stop your MySQL server";
        echo -e "\t\t3: Delete everything in the MySQL data directory (usually /var/lib/mysql)";
        echo -e "\t\t4: xtrabackup --copy-back $backupDir/(daily/weekly/monthly)/BACKUP-DIR";
        ...
        echo -e "\t3: xtrabackup --apply-log --redo-only $backupDir/(daily/weekly/monthly)/FULL-BACKUP-DIR";
        ...
        echo -e "\t6: xtrabackup --apply-log --redo-only $backupDir/(daily/weekly/monthly)/FULL-BACKUP-DIR";
        ...
        echo -e "\t8: xtrabackup --apply-log $backupDir/(daily/weekly/monthly)/BACKUP-DIR";
...
if [ $tomorrow -eq 1 ]; then
        ...
        if [ $difference -gt $hoursBeforeFull ]; then
                /usr/bin/xtrabackup --user=$mysqlUser --password=$mysqlPwd --no-timestamp $compress $compressThreads --tmpdir=$tmpDir --rsync "$monthlyDir"/"$dateNow" > $backupLog 2>&1
        ...
        # Check if we must take a full or incremental backup
        if [ $difference -gt $hoursBeforeFull ]; then
                /usr/bin/xtrabackup --user=$mysqlUser --password=$mysqlPwd --no-timestamp $compress $compressThreads --tmpdir=$tmpDir --rsync "$weeklyDir"/"$dateNow" > $backupLog 2>&1
                echo $dateNowUnix > "$weeklyDir"/last_weekly
		...
		if [[ $incrtype == full ]]; then
			...
			/usr/bin/xtrabackup --user=$mysqlUser --password=$mysqlPwd --no-timestamp $compress $compressThreads --rsync --tmpdir=$tmpDir --incremental --incremental-basedir="$dailyDir"/"$lastIncrDir"_incr "$dailyDir"/"$dateNow"_incr > $backupLog 2>&1
                ...
                echo $dateNowUnix > "$dailyDir"/latest_full
                /usr/bin/xtrabackup --user=$mysqlUser --password=$mysqlPwd --no-timestamp $compress $compressThreads --tmpdir=$tmpDir --rsync "$dailyDir"/"$dateNow"_full > $backupLog 2>&1
        fi
fi
